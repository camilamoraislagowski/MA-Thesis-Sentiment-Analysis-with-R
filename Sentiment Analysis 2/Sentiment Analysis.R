...
## TOKENIZATION
news_df <- test_data %>% select(headline_text)
...
news_tokens_count %>% ggplot(aes(word, proportion*100, fill=ceiling(proportion*100))) + geom_col() + xlab(NULL) + coord_flip() + theme(legend.position="none")

# OUTPUT CHECK
head(news_df, 10)

news_df_subset <- news_df[1:1000,,drop=FALSE]
tkn_1 <- apply(news_df_subset, 1, function(x) { data.frame(headline_text=x, stringsAsFactors =FALSE) %>% unnest_tokens(word,headline_text)})

single_news_tokens <- lapply(tkn_1, function(x) {anti_join(x, stop_words)})
str(single_news_tokens, list.len=5)

single_news_tokens[[1]]


###SENTIMENT ANLAYSIS USING DIFFERENT LEXICONS
