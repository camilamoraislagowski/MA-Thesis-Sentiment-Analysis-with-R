...
head(FinancialTimesData)
tail(FinancialTimesData)

## TOKENIZATION -> CHANGE THE DATA FILE TO CHANGE THE NEWS PROVIDER
news_df <- FinancialTimesData %>% select(headline_text)
...
min_frequency <- 36  # Adjust as needed 
...
ggplot(filtered_words, aes(x = log_n, y = rank, size = n, label = word)) +
  ...
  labs(title = "FT Word Map with Frequency (Log Scale) and Order of Evocation",
       x = "Log Frequency",
       y = "Rank") +
  theme_minimal() +
