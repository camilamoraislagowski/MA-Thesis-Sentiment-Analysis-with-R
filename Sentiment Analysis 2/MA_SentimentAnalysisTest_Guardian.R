...
head(TheGuardianData)
tail(TheGuardianData)

## TOKENIZATION -> CHANGE THE DATA FILE TO CHANGE THE NEWS PROVIDER
news_df <- TheGuardianData %>% select(headline_text)
...
min_frequency <- 35  # Adjust as needed 
...
ggplot(filtered_words, aes(x = log_n, y = rank, size = n, label = word)) +
  ...
  labs(title = "Guardian Word Map with Frequency (Log Scale) and Order of Evocation",
       x = "Log Frequency",
       y = "Rank") +
  theme_minimal() +
